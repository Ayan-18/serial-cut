...
// Backend timestamps are UTC; SQLite drops the offset on some rows, so a tz-less
// string must be read as UTC, not the browser's local zone.
export function parseServerDate(value: string) { return new Date(/([zZ]|[+-]\d\d:?\d\d)$/.test(value) ? value : `${value}Z`); }
export function elapsedFrom(value: string) { return formatElapsed(Math.max(0, Math.floor((Date.now() - parseServerDate(value).getTime()) / 1000))); }
