...
После `Найти лица` сохраняется сглаженная временная траектория, и FFmpeg плавно перемещает crop по
ходу ролика. Персонажа можно выбрать из списка в каждой строке субтитров; настройка `Показывать имя
