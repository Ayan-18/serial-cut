...
В обоих режимах — **«Центр»** и **«По лицу»** — основное видео занимает две трети высоты
вертикального кадра: 1080x1280 по центру, сверху и снизу по 320 px затемнённого размытого фона.
Для исходника 16:9 при масштабе 1 это показывает в 1,5 раза больше сцены по ширине, чем прежнее
кадрирование на всю высоту. «Центр» оставляет выбранное положение, «По лицу» использует траекторию
трекинга. «Приближение внутри кадра» увеличивает изображение, но не убирает фоновые поля.
Предпросмотр, обычный экспорт и StoryArc используют одну компоновку. Старое значение
`blurred-background` читается как «Центр»; ранее экспортированные файлы не изменяются.
Для новой компоновки перезапустите приложение, обновите страницу и запустите рендер повторно.
...
подтверждённое имя над текстом в готовом видео. Настройка `Анимированные субтитры`
(`subtitle_animate`, по умолчанию выключена) включает караоке-режим: слово подсвечивается по мере
произношения (тайминг — из word-timestamps Whisper, иначе равномерно). Стиль субтитров входит в
render fingerprint, поэтому смена настройки создаёт новую версию MP4.
