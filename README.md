...
- `GET /api/model-catalog` показывает локальные модели (ASR, Qwen, лица, озвучка Silero): размер,
  папку, статус установки и точную команду. Небольшие модели (лица, Silero ~60 МБ) можно скачать
  прямо из панели после подтверждения (`POST /api/model-catalog/{key}/install`); Whisper и Qwen
  по-прежнему ставятся скриптом. Для Silero нужен опциональный пакет `torch`:
  `pip install -e ".[tts]"`.
