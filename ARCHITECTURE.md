...
- `media/face_tracking.py` строит траекторию crop из трёх проходов: (1) плотная выборка кадров с
  ...
  короткие реплики (< `_MIN_DWELL_SECONDS`) вклеиваются в соседний план, а на смене говорящего
  ставится жёсткий рез (пара keyframe в `CUT_GAP_SECONDS`), внутри плана — плавное следование.
