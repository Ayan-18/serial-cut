...
- `subtitles.py` форматирует русские SRT/ASS для мобильного просмотра; при `subtitle_animate`
  `render_ass` строит караоке-ASS (одно событие на слово, активное слово подсвечивается и
  «подпрыгивает»), тайминг берётся из `SubtitleCue.word_times`, иначе — равномерная развёртка.
...
- `media/face_tracking.py` строит траекторию crop из трёх проходов: (1) плотная выборка кадров с
  ...
  короткие реплики (< `_MIN_DWELL_SECONDS`) вклеиваются в соседний план. Каждый кадр даёт цель по x
  (лицо говорящего, если план уверенный по `_CONFIDENT_RUN_FRACTION`; иначе — центр), которая
  прогоняется через критически задемпфированную пружину `_spring_step` (`_FOLLOW_FREQ_HZ`): crop
  плавно едет за говорящим, стоит когда стоит он (мёртвая зона `_DEAD_ZONE`), на смене спикера —
  спокойный ~0.5-секундный доворот без перелёта. `_center_offset` учитывает аспект исходника и
  ставит лицо на `_CENTERING_FRACTION` пути к центру окна 1080×1280. `media/speakers.py` выполняет
  первичную кластеризацию реплик.
