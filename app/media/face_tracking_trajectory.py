"""The sample-analysis and camera-trajectory engine behind face_tracking.py.

None of this touches `LocalFaceRecognizer`/`build_reference_vectors` (the two
names a test monkeypatches on `app.media.face_tracking`), so it was safe to
split out of that module: `estimate_face_offset` stays there and calls into
this module once it already has per-frame face detections in hand.
"""
...
from app.media.character_recognition import FaceObservation, best_character_match

if TYPE_CHECKING:
    # Only for the type checker: SpeechRange lives in face_tracking.py, which
    # imports this module, so importing it for real here would be circular.
    from app.media.face_tracking import SpeechRange
...
# few callers still import the name (re-exported from face_tracking.py too).
...
def _sample_times(
    start_time: float, end_time: float, ranges: list, samples: int | None
) -> list[float]:
...
def _active_speech(timestamp: float, ranges: list):
